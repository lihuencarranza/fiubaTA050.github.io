---
layout: page
title: Calendario
permalink: /calendario/
---

<table class="calendario">
  <thead>
    <tr>
      <th>Día</th>
      <th>Fecha</th>
      <th>Tema</th>
      <th>Lecturas</th>
      <th>Notas</th>
      <th>Video</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Miércoles</td>
      <td>11/3</td>
      <td>
        <ul class="cal-temas">
          <li>Introducción</li>
          <li>Modelo Cliente-Servidor</li>
          <li>Remote Procedure Call</li>
        </ul>
      </td>
      <td><a href="https://web.stanford.edu/class/ee384m/Handouts/HowtoReadPaper.pdf">How to read a paper - Keshav</a></td>
      <td><a href="https://drive.google.com/file/d/1nvDap74mlYvCX81Cc1maEGsZ_fKwhrgu/view?usp=drive_link"><i class="fas fa-file-pdf fa-lg"></i></a></td>
      <td><a href="https://youtu.be/M5nVa0z5FGQ"><i class="fas fa-file-video fa-lg"></i></a></td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>13/3</td>
      <td>
        <ul class="cal-temas">
          <li>MapReduce</li>
          <li><strong>TP1 - Enunciado</strong></li>
        </ul>
      </td>
      <td><a href="https://static.googleusercontent.com/media/research.google.com/es//archive/mapreduce-osdi04.pdf">MapReduce - Dean</a></td>
      <td><a href="https://drive.google.com/file/d/1kLjeiW77cAGTCqIfxOtwWB5KTHvVQu68/view?usp=drive_link"><i class="fas fa-file-pdf fa-lg"></i></a></td>
      <td><a href="https://youtu.be/rZEyNErIXbE"><i class="fas fa-file-video fa-lg"></i></a></td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>18/3</td>
      <td>
        <ul class="cal-temas">
          <li>Introducción a Go</li>
        </ul>
      </td>
      <td><a href="https://go.dev/tour">A Tour of Go</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>20/3</td>
      <td>
        <ul class="cal-temas">
          <li>Máquina de estado distribuida</li>
          <li>Master/Backup replication</li>
        </ul>
      </td>
      <td><a href="https://engineering.linkedin.com/distributed-systems/log-what-every-software-engineer-should-know-about-real-time-datas-unifying">The Log - Kreps</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>25/3</td>
      <td>
        <ul class="cal-temas">
          <li>Google Filesystem</li>
        </ul>
      </td>
      <td><a href="https://static.googleusercontent.com/media/research.google.com/es//archive/gfs-sosp2003.pdf">GFS - Ghemawat</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>27/3</td>
      <td></td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-entrega">
      <td>Miércoles</td>
      <td>1/4</td>
      <td>
        <ul class="cal-temas">
          <li><strong><span style="color: #c00;">TP1 - Entrega y presentación</span></strong></li>
        </ul>
      </td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-feriado">
      <td>Viernes</td>
      <td>3/4</td>
      <td>
        <ul class="cal-temas">
          <li><strong>FERIADO</strong></li>
        </ul>
      </td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>8/4</td>
      <td>
        <ul class="cal-temas">
          <li>Raft - Parte I - Elección de líder</li>
          <li><strong>TP2 - Enunciado</strong></li>
        </ul>
      </td>
      <td><a href="https://pdos.csail.mit.edu/6.824/papers/raft-extended.pdf">Raft - Ongaro</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>10/4</td>
      <td>
        <ul class="cal-temas">
          <li>Raft - Parte II - Log y Snapshots</li>
        </ul>
      </td>
      <td><a href="https://pdos.csail.mit.edu/6.824/papers/raft-extended.pdf">Raft - Ongaro</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-entrega">
      <td>Miércoles</td>
      <td>15/4</td>
      <td>
        <ul class="cal-temas">
          <li>Linealizabilidad</li>
          <li>Zookeeper</li>
          <li><strong><span style="color: #c00;">TP2 - Entrega 2A</span></strong></li>
        </ul>
      </td>
      <td><a href="https://www.usenix.org/legacy/event/atc10/tech/full_papers/Hunt.pdf">Zookeeper - Hunt</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>17/4</td>
      <td></td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-entrega">
      <td>Miércoles</td>
      <td>22/4</td>
      <td>
        <ul class="cal-temas">
          <li>Consistencia en Caches</li>
          <li><strong><span style="color: #c00;">TP2 - Entrega 2B</span></strong></li>
        </ul>
      </td>
      <td><a href="https://pdos.csail.mit.edu/6.824/papers/memcache-fb.pdf">Memcache - Nishtala</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>24/4</td>
      <td></td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-entrega">
      <td>Miércoles</td>
      <td>29/4</td>
      <td>
        <ul class="cal-temas">
          <li>Dynamo - Parte I</li>
          <li>Hashing consistente</li>
          <li>Relojes de Lamport</li>
          <li>Relojes vectoriales</li>
          <li><strong><span style="color: #c00;">TP2 - Entrega 2C</span></strong></li>
        </ul>
      </td>
      <td><a href="https://www.amazon.science/publications/dynamo-amazons-highly-available-key-value-store">Dynamo - DeCandia</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-feriado">
      <td>Viernes</td>
      <td>1/5</td>
      <td>
        <ul class="cal-temas">
          <li><strong>FERIADO</strong></li>
        </ul>
      </td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-entrega">
      <td>Miércoles</td>
      <td>6/5</td>
      <td>
        <ul class="cal-temas">
          <li>Dynamo - Parte II</li>
          <li>Gossip protocols</li>
          <li>Merkle Trees</li>
          <li>DynamoDb</li>
          <li><strong><span style="color: #c00;">TP2 - Entrega 2D</span></strong></li>
        </ul>
      </td>
      <td><a href="https://www.usenix.org/system/files/atc22-elhemali.pdf">DynamoDb - Elhemali</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>8/5</td>
      <td>
        <ul class="cal-temas">
          <li>Transacciones distribuidas</li>
          <li><strong>TP3 - Enunciado</strong></li>
        </ul>
      </td>
      <td><a href="https://www.usenix.org/system/files/atc23-idziorek.pdf">DynamoDb - Idziorek</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>13/5</td>
      <td>
        <ul class="cal-temas">
          <li>Cloud databases I - Aurora (Amazon)</li>
        </ul>
      </td>
      <td><a href="http://nil.csail.mit.edu/6.824/2020/papers/aurora.pdf">Aurora - Verbitski</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>15/5</td>
      <td>
        <ul class="cal-temas">
          <li>Cloud databases II - Spanner (Google)</li>
        </ul>
      </td>
      <td><a href="http://nil.csail.mit.edu/6.5840/2025/papers/spanner.pdf">Spanner - Corbett</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>20/5</td>
      <td>
        <ul class="cal-temas">
          <li>Spark <em>(docente invitado)</em></li>
        </ul>
      </td>
      <td><a href="http://nil.csail.mit.edu/6.5840/2023/papers/zaharia-spark.pdf">Spark - Zaharia</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>22/5</td>
      <td>
        <ul class="cal-temas">
        </ul>
      </td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>27/5</td>
      <td>
        <ul class="cal-temas">
          <li>Message Oriented Middleware</li>
          <li>Kafka</li>
        </ul>
      </td>
      <td><a href="https://www.microsoft.com/en-us/research/wp-content/uploads/2017/09/Kafka.pdf">Kafka - Kreps</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>29/5</td>
      <td>
        <ul class="cal-temas">
          <li>Data Streams processing<sup><a href="#fn1" id="fnref1">1</a></sup></li>
        </ul>
      </td>
      <td><a href="https://research.google/pubs/millwheel-fault-tolerant-stream-processing-at-internet-scale/">Millwheel - Akidau</a><br><a href="https://static.googleusercontent.com/media/research.google.com/es//pubs/archive/43864.pdf">Dataflow - Akidau</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>3/6</td>
      <td>
        <ul class="cal-temas">
          <li>Práctica: Kafka, Flink y Beam</li>
        </ul>
      </td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr class="cal-entrega">
      <td>Viernes</td>
      <td>5/6</td>
      <td>
        <ul class="cal-temas">
          <li><strong><span style="color: #c00;">TP3 - Entrega y presentación</span></strong></li>
        </ul>
      </td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>10/6</td>
      <td>
        <ul class="cal-temas">
          <li>Bitcoin <em>(docente invitado)</em></li>
        </ul>
      </td>
      <td><a href="https://pdos.csail.mit.edu/6.824/papers/bitcoin.pdf">Bitcoin - Nakamoto</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>12/6</td>
      <td></td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>17/6</td>
      <td>
        <ul class="cal-temas">
          <li>Cluster computing</li>
          <li>Warehouse-scale computer<sup><a href="#fn2" id="fnref2">2</a></sup></li>
          <li>Borg</li>
        </ul>
      </td>
      <td><a href="http://nil.csail.mit.edu/6.824/2016/papers/borg.pdf">Borg - Verma</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>19/6</td>
      <td>
        <ul class="cal-temas">
          <li>Serverless computing</li>
          <li>Lambda (Amazon)</li>
        </ul>
      </td>
      <td><a href="https://pdos.csail.mit.edu/6.824/papers/atc23-brooker.pdf">Lambda</a> <a href="https://www.usenix.org/system/files/nsdi20-paper-agache.pdf">Firecracker</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Miércoles</td>
      <td>24/6</td>
      <td></td>
      <td></td>
      <td>—</td>
      <td>—</td>
    </tr>
    <tr>
      <td>Viernes</td>
      <td>26/6</td>
      <td>
        <ul class="cal-temas">
          <li>Internet Scale Services</li>
        </ul>
      </td>
      <td><a href="https://s3.amazonaws.com/systemsandpapers/papers/hamilton.pdf">Services - Hamilton</a></td>
      <td>—</td>
      <td>—</td>
    </tr>
  </tbody>
</table>

<footer class="calendario-notas">
  <p id="fn1"><sup>1</sup> <a href="https://www.amazon.com/-/es/Streaming-Systems-Where-Large-Scale-Processing/dp/1491983876">Streaming Systems</a> by Tyler Akidau</p>
  <p id="fn2"><sup>2</sup> <a href="https://www.amazon.es/Datacenter-Computer-Designing-Warehouse-Scale-Architecture/dp/1681734338">The Datacenter as a Computer</a> by Barroso</p>
</footer>
